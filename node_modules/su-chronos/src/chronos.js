...
Chronos - A node-based hybrid server for serving full stack applications.
...
function stdout(string, color, shouldBreak) {
  process.stdout.write(
    `${color ? color : ""}${string}${ansi.reset}${shouldBreak ? "\n" : ""}`
  );
  return void 0;
}
...
export default class server {
  ...
  /**
   * Build the chronos application.
   */
  build() {
    const chronosFile = join(this.workingDirectory, "Chronosfile");
  ...
  /**
   * Build and start the chronos application.
   */
  start() {
    this.build();
    this.startServer();
